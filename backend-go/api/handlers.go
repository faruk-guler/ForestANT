...
func SetupRoutes(app *fiber.App) {
	// ACME Challenge Route — FIX: thread-safe GetChallenge kullan
	app.Get("/.well-known/acme-challenge/:token", func(c *fiber.Ctx) error {
		token := c.Params("token")
		if keyAuth, ok := acme.GetChallenge(token); ok {
	...
	api.Post("/domains", func(c *fiber.Ctx) error {
		...
		// FIX: SQLite sync
		database.DB.Exec("INSERT OR IGNORE INTO domains (hostname, status) VALUES (?, ?)", hostname, "pending")
	...
	api.Post("/domains/bulk", func(c *fiber.Ctx) error {
		...
		if len(toAdd) > 0 {
			...
			// FIX: SQLite bulk sync
			for _, d := range toAdd {
				database.DB.Exec("INSERT OR IGNORE INTO domains (hostname, status) VALUES (?, ?)", d.Hostname, "pending")
			}
	...
	api.Post("/domains/bulk-delete", func(c *fiber.Ctx) error {
		...
		for _, id := range payload.Ids {
			// Get hostname before deleting to delete from SQLite too
			var hostname string
			for _, d := range DomainStorage.GetAll() {
				if d.ID == id {
					hostname = d.Hostname
			...
			// FIX: SQLite sync
			if hostname != "" {
				database.DB.Exec("DELETE FROM domains WHERE hostname = ?", hostname)
			}
		}
	...
	api.Put("/domains/:id", func(c *fiber.Ctx) error {
		...
		hostname := strings.ToLower(strings.TrimSpace(payload.Hostname))
		// FIX: PUT'ta da format doğrulama
		if !domainRegex.MatchString(hostname) {
			return c.Status(400).JSON(fiber.Map{"error": "Invalid hostname format"})
		}
	...
	api.Post("/settings/test-webhook", func(c *fiber.Ctx) error {
		notifications.SendNotification("✅ **Test Başarılı!** ForestANT Go Webhook bağlantınız düzgün çalışıyor.")
	...
	// FIX: Eksik DELETE /workflows/:id endpoint eklendi
