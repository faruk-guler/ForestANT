...
const WorkflowList: React.FC<Props> = ({ workflows, domains, accessList, fetchWorkflows, apiUrl }) => {
  ...
  const [remotePath, setRemotePath] = useState('/etc/ssl/certs');       // FIX: kullanıcı düzenleyebilir
  const [reloadCmd, setReloadCmd] = useState('systemctl reload nginx');  // FIX: kullanıcı düzenleyebilir
  ...
  const handleAdd = async () => {
    try {
      await axios.post(`${apiUrl}/workflows`, {
        ...
        access_id: accessId,
        type: type,
        // FIX: remote_path ve reload_command artık kullanıcı girdisinden geliyor
        config: JSON.stringify({ remote_path: remotePath, reload_command: reloadCmd })
      });
  ...
  // FIX: Silme fonksiyonu ve eksik endpoint bağlantısı eklendi
  const handleDelete = async (id: number) => {
    try {
      ...
      addToast('Error deleting workflow', 'error');
    }
  };
    ...
    <div className="card" style={{ marginTop: '1rem' }}>
      ...
      {showAdd && (
        <div style={{ padding: '1.5rem', borderBottom: '1px solid var(--border)', background: 'rgba(59, 130, 246, 0.02)' }}>
          ...
          <div className="grid-2" style={{ marginTop: '1rem' }}>
            <div className="form-group">
              <label>Remote Cert Path</label>
              <input value={remotePath} onChange={e => setRemotePath(e.target.value)} placeholder="/etc/ssl/certs" />
            </div>
            <div className="form-group">
              <label>Reload Command</label>
              <input value={reloadCmd} onChange={e => setReloadCmd(e.target.value)} placeholder="systemctl reload nginx" />
            </div>
          </div>
              ...
              <tr key={w.id}>
                ...
                <td style={{ textAlign: 'right' }}>
                  <div className="actions" style={{ justifyContent: 'flex-end' }}>
                    ...
                    {/* FIX: Butonlar artık bağlı */}
                    <button className="btn-icon text-danger" title="Delete Workflow" onClick={() => handleDelete(w.id)}>
                      <Trash2 size={16} />
                    </button>
                  </div>
                </td>
